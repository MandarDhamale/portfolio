document.addEventListener('DOMContentLoaded', function() {
    ...
    function addEyeIconToFooter() {
        ...
        const footerCVButton = document.querySelector('.cv-floating-button, .download-cv');
        ...
        if (!targetButton) {
            allFooterButtons.forEach(btn => {
                ...
                if (rect.top > window.innerHeight * 0.7) { // If button is in the bottom 30% of viewport
                    targetButton = btn;
                }
            });
        }
        ...
        if (targetButton && !targetButton.querySelector('.footer-eye-btn') && 
            ...
            eyeBtn.innerHTML = '<i class="fa-regular fa-eye" style="font-size: 14px; color: #333;"></i>';
    ...
    document.addEventListener('click', function(e) {
        ...
        if (e.target.classList.contains('eye-preview-btn') || 
            (e.target.parentNode && e.target.parentNode.classList.contains('eye-preview-btn'))) {
            openResumePreview();
        }
    });
    ...
    if (typingElement && typeof Typed !== 'undefined') {
        new Typed('.input', {
            strings: ["A Tech Enthusiast", "A Software Engineer", "A Problem Solver", "An AI/ML Explorer"],
            typeSpeed: 70,
            backSpeed: 55,
            loop: true
        });
